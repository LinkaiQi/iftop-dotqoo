...
"                                        q - quit\n"\
"Sorting:                               dotqoo:\n"\
" 1/2/3 - sort by 1st/2nd/3rd column     F - save packet to file(/tmp)\n"\
" < - sort by source name                w - write info to file(/tmp)\n"\
" > - sort by dest name                  c - send info to socket\n"\
...
void ui_print() {
	...
	//2017.01.19-isshe------------------
	sorted_list_type temp_list = screen_list;
	if (options.write_communication_info != 0 || options.send_communication_info != 0)
	{
	  	while((nn = sorted_list_next_item(&temp_list, nn)) != NULL)
	  	{

		  	  host_pair_line* screen_line = (host_pair_line*)nn->data;
			  //\CC\EE\B3\E4\BDṹ
			  memset(&info, '\0', sizeof(communication_info_s));
			  inet_ntop(screen_line->ap.af, &(screen_line->ap.src6), info.src_addr, sizeof(info.src_addr));
			  inet_ntop(screen_line->ap.af, &(screen_line->ap.dst6), info.dst_addr, sizeof(info.dst_addr));
			  info.src_port = screen_line->ap.src_port;
			  info.dst_port = screen_line->ap.dst_port;
			  readable_size(screen_line->total_sent, info.tx_total, 10, 1024, 1);
			  readable_size(screen_line->total_recv, info.rx_total, 10, 1024, 1);
...
void ui_init() {
    ...
    erase(); 			//\D5\E2\C0\EFӦ\B8\C3\CA\C7\C7\E5\C0\ED\BD\E7\C3\E6
    ...
    while(foad == 0) {
        ...
        switch (i) {
            ...
            case 'L':
                ...
                ui_print();
                break;
			//\CC\ED\BC\D3'F':save file, 2017.01.17-isshe
			case 'F': 	//write packet to file
				options.save_file = !options.save_file;
				if(options.save_file){
					showhelp("Save file enable(/tmp)");
				}
				else {
					showhelp("Save file disable");
				}
				break;
			case 'w': 	//write info to file
				options.write_communication_info = !options.write_communication_info;
				options.write_interval_timeout = 0;
				if (options.write_communication_info)
				{
					showhelp("write info to file enable(/tmp)");
				}
				else
				{
					showhelp("write info to file disable");
				}
				break;
			case 'c': 	//send info to socket
				options.send_communication_info = !options.send_communication_info;
				options.create_send_thread = 0;
				if (options.send_communication_info)
				{
					showhelp("send info to socket enable(/tmp)");
				}
				else
				{
					showhelp("send info to socket disable");
				}
				break;
            case KEY_CLEAR:
            case 12:    /* ^L */
